...
void ::Murmur::ServerI::reloadCertificate_async(const ::Murmur::AMD_Server_reloadCertificatePtr &cb, const ::Ice::Current &current) {
	// qWarning() << "reloadCertificate" << meta->mp.qsIceSecretRead.isNull() << meta->mp.qsIceSecretRead.isEmpty();
#ifndef ACCESS_Server_reloadCertificate_ALL
#ifdef ACCESS_Server_reloadCertificate_READ
	if (! meta->mp.qsIceSecretRead.isNull()) {
		bool ok = ! meta->mp.qsIceSecretRead.isEmpty();
#else
	if (! meta->mp.qsIceSecretRead.isNull() || ! meta->mp.qsIceSecretWrite.isNull()) {
...
#endif
	ExecEvent *ie = new ExecEvent(boost::bind(&impl_Server_reloadCertificate, cb, QString::fromStdString(current.id.name).toInt()));
	QCoreApplication::instance()->postEvent(mi, ie);
}
