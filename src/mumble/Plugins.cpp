	...
	foreach (const QString &entry, entries) {
		...
		if (baseName == QLatin1String("wine-preloader") || baseName == QLatin1String("wine64-preloader")) {
			...
			if (f.open(QIODevice::ReadOnly)) {
				...
				if (exe.contains(QLatin1String("/"))) { // Absolute path, find basename...
					int lastSlash = exe.lastIndexOf(QLatin1String("/"));
					if (exe.count() > lastSlash + 1) {
						baseName = exe.mid(lastSlash + 1);
					}
				} else if (exe.contains(QLatin1String("\\"))) { // Windows-style path (Wine)
